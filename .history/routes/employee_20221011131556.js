...
router.post('/apply-for-leave', function sendMail(req, res, next) {
    ...
    newMail.applicantID = req.user._id;
    newMail.title = req.body.title;
    newMail.email = req.session.user.email,
    newMail.userName = req.session.user.name;
    newMail.csrfToken = req.csrfToken();
