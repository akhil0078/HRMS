...
router.post('/apply-for-leave/mailer', function sendMail(req, res, next) {

    User.findById(req.session.user._id, function sendMail(err, user) {
        if (err) {
            console.log(err);
        }
        res.sendMail({
